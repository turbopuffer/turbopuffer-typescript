...
import { Turbopuffer, ClientOptions } from '@turbopuffer/turbopuffer';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Turbopuffer)`
    : `const run: (${functionSource.client}: Turbopuffer) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.namespaces.metadata',
    'client.namespaces.multiQuery',
