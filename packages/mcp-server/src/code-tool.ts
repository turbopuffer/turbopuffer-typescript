...
const remoteStainlessHandler = async (
  ...
  TURBOPUFFER_API_KEY: requireValue(readEnv('TURBOPUFFER_API_KEY') ?? client.apiKey, 'set TURBOPUFFER_API_KEY environment variable or provide apiKey client option'),
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: "turbopuffer",
      code,
      intent,
      client_opts: { defaultNamespace: readEnv('TURBOPUFFER_DEFAULT_NAMESPACE') },
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@turbopuffer/turbopuffer';
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
          ...
          ...(client.region != null ? { region: client.region } : undefined),
          ...(client.defaultNamespace != null ? { defaultNamespace: client.defaultNamespace } : undefined),
          defaultHeaders: {
