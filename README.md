...
```sh
npm install git+ssh://git@github.com:stainless-sdks/turbopuffer-typescript.git
```
...
async function main() {
  const response = await client.namespaces.upsert('products', { documents: {} });
...
Note that if given a `TURBOPUFFER_LOG=debug` environment variable, this library will log all requests and responses automatically.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/turbopuffer-typescript/issues) with questions, bugs, or suggestions.
